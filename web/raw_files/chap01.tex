...
这里先讲一下smartphone和smartphone usage？


随着信息技术的不断发展,如今,智能手机已经成为人们生活中必不可少的移动电子设备。与此同时,智能手机也是日常生活中人们最频繁的交互设备之一,对交互效率、性能与交互体验也有相对较高的要求。目前，电容屏幕的触摸信号仍然是智能手机输入的主要模态。然而，单一的电容信号输入存在许多问题与交互的不便。例如：

\begin{itemize}
    \item 误触问题：手机直接接收的输入信号为二维电容信号序列，因此无法有效理解当前信号是否由有输入意图的用户行为产生。
    \item 操作的自然性与舒适性问题：用户的操作舒适性会受到设备屏幕大小、用户界面设计等因素影响。
    \item 交互效率问题：单一的触摸信号能传达的信息十分有限，若要表达更丰富的信息，需要在时间、空间维度对其进行组合与扩充，但这些扩充仍然受限于延迟、手势集的可扩展性等问题。\cite{Chen:2014:AIT:2642918.2647392}
\end{itemize}

针对上述问题，现有的绝大多数设计与解决方案均为针对特定场景或任务的，balabala。这些针对特定场景的算法的设计思想可以分为两种：
\begin{itemize}
    \item 从现有硬件出发：不引入额外的传感器与传感信息，而是从现有传感设备所捕获的原始数据中进一步地挖掘出关键信息（例如通过屏幕电容信号的形状检测误触\ref{}、电容信号与IMU（运动传感器）数据结合实现交互增强）。
    \item 从交互需求出发：通过额外的外置传感器，例如外置深度摄像头、环绕式灵敏电容传感器、声学传感器等，来捕获算法所需要的传感数据。
\end{itemize}

这两种设计思想均有各自的局限性：对于前者，虽然其充分地挖掘与利用了已有的传感信息，但是客观上其获取信息是不完整的，有些模态的信息是不能通过内置传感器直接或间接获取的（例如图像、屏外手部动作），因此这部分信息无法被用来支持交互；对于后者，引入格外的感测系统或额外的传感器会带来额外的硬件成本以及体积、便携性、实用性等负面影响，因此多数增加外置传感器的实验系统仅能用于验证交互设计。

HandSee通过在手机前置摄像头上方放置一面三棱镜，原像与镜面反射像构成有一定视差的双目视觉系统，使其能够捕捉用户操作手机时的手部深度图像，并进一步地基于此完成一系列应用。该工作很好地结合了上述两种思想，利用手机内置硬件，通过简单的改造，大幅增强手机对用户交互行为的感知能力，为实现更智能、用户体验更好交互提供了信息感知的支持。然而其局限性在于其运算流程在服务器端使用GPU完成，流程设计未充分考虑计算量与计算效率问题，且深度图重构部分噪音较大、指尖识别鲁棒性仍有欠缺，这部分将会在文献综述中详细阐述。
